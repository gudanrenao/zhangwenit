...
import com.zhangwen.learn.zhangwenit.concurrent.util.Helpers;
...
public class ConcurrentLinkedQueue2<E> extends AbstractQueue<E>
    ...
    public boolean offer(E e) {
        ...
        for (Node<E> t = tail, p = t; ; ) {
            //此时t和p都指向tail，q指向tail的next节点
            Node<E> q = p.next;
            //如果tail.next节点为空，那么p(tail)就是最后一个元素，也是head队列的最后一层的next元素
            if (q == null) {
                // p is last node
                //将入队元素cas设置到tail的next节点上，同时也是head最后一层next元素设置其next为当前入队元素
    ...
    static {
        try {
            ...
            HEAD = l.findVarHandle(ConcurrentLinkedQueue2.class, "head",
                    Node.class);
            TAIL = l.findVarHandle(ConcurrentLinkedQueue2.class, "tail",
                    Node.class);
