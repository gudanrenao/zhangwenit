...
/**
 * @Description ThreadPoolExecutor 过程注释
 * @Author ZWen
 * @Date 2019/4/9 1:43 PM
 * @Version 1.0
 **/
public class MyThreadPoolExecutor extends AbstractExecutorService {
     ...
     * holding mainLock.
     */
    private final HashSet<MyThreadPoolExecutor.Worker> workers = new HashSet<>();
     ...
     * <p>
     * 设置创建线程的工厂，可以通过工厂给每个线程设置更有意义的名字，可以使用开源框架
     * guava的ThreadFactoryBuilder快速给线程设置更有意义的名字
     * 或者使用BasicThreadFactory等
     */
     ...
     * <p>
     * 默认提供四种饱和策略(对于自己的业务可以自己实现一个，比如记录日志或持久化到数据库)
     * 1.AbortPolicy:           直接抛出异常
     * 2.DiscardOldestPolicy:   丢弃任务队列中最近的一次任务，并执行当前任务
     * 3.DiscardPolicy:         不处理，直接丢弃当前任务
     * 4.CallerRunsPolicy:      由调用者所在线程来执行任务(同步执行)
     */
     ...
     * <p>
     * 空闲线程超时时间.当线程数量大于基本数量或allowCoreThreadTimeOut时，
     * 线程使用此超时。否则，他们将永远等待新任务
     * 也就是说该时间用于非基本线程或allowCoreThreadTimeOut为true时的基本线程的闲置超时时间
     */
     ...
     * <p>
     * 线程池基本大小，提交一个任务时会创建一个线程来执行，即使其他空闲的基本线程
     * 能够执行该任务也会创建，直到执行任务数大于等于线程池基本大小时才不会创建
     * 如果调用了prestartAllCoreThreads()，线程池会提前创建并启动所有基本线程
     */
     ...
     * <p>
     * 线程池允许创建的最大线程数，如果队列满了且已创建的线程数小于最大线程数，
     * 那么线程池会创建新的线程执行任务
     * 注意:如果使用了无界的任务队列这个参数就没有用了！！！
     */
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (int c = ctl.get(); ; ) {
            ...
            //如果有工作线程，且(工作线程大于最大值或任务不为空或工作队列为空)，那么添加Worker失败
            //任务不为空的判断，感觉是为了初始化线程池(提前创建好核心线程，prestartAllCoreThreads())
        ...
        try {
            //Worker(工作线程,继承AQS),创建线程时，会将线程封装为工作线程Worker，Worker在执行完任务后，
            //会循环获取工作队列中的任务来执行，可以在Worker#run()中验证
            ...
            if (t != null) {
                ...
                try {
                    ...
                    if (isRunning(c) ||
                        ...
                        //检测t是否是可启动的，也就是没执行过start(),属于NEW状态，如果不是抛出异常
    ...
    public void execute(Runnable command) {
        ...
        //如果线程数>=基本线程数(一般预热后，execute都是从这儿开始)或线程创建失败，则将当前任务
        //添加到工作队列中
        ...
        //如果线程池不处于运行中或任务无法加入队列中，则创建一个线程执行任务
